...
class ProjectsController < ApplicationController
  ...
  def index
    if params[:user_id].nil?
      @projects = Project.joins(:project_languages).where('project_languages.language_id' => current_user.language_ids).where.not('user_id' => current_user.id).group('project_id').reverse
    else
